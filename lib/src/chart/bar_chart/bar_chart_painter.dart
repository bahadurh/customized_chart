...
class BarChartPainter extends AxisChartPainter<BarChartData> {
  ...
  void paint(BuildContext context, CanvasWrapper canvasWrapper,
    ...
    List<double> groupPositon = [];

    _groupBarsPosition = calculateGroupAndBarsPosition(
        canvasWrapper.size, groupsX, data.barGroups);
    _groupBarsPosition?.forEach((element) {
      groupPositon.add(element.groupX);

      //print("bar x => ${element.groupX}");
      // path.addRRect(
      //     RRect.fromRectXY(Rect.fromLTRB(element.groupX+20 , -8, element.groupX - 25, 150), 10, 10));
    ...
    for (var i = 0; i < targetData.barGroups.length; i++) {
      ...
      for (var j = 0; j < barGroup.barRods.length; j++) {
        ...
        if (barGroup.isSelected) {
          path.addRRect(RRect.fromRectXY(
              Rect.fromLTRB(groupPositon[i] + 20, -15, groupPositon[i] - 20,
                  size.height + 50),
              21.r,
              21.r));
          canvasWrapper.drawPath(path, paint);
